...
create_departments_table = 'CREATE TABLE Departments([Id] [int] NOT NULL,[Code] [nvarchar](200) NOT NULL,[DepartmentName] [nvarchar](200) NOT NULL,[UnitName] [nvarchar](10) NULL,[TotalSeats] [int], [AllottedSeats] [int], [SeatStatus] [bit], [UpdatedDate] [nvarchar](450) NULL)'
cursor.execute(create_departments_table)

connection_string.commit()

# Copy data of applicants who have filled up choice form
copy_from_applications_table = "SELECT Id, UserId, ApplicationId, [Position], Roll, UnitName, TotalMarks FROM Admission2019.Applications WHERE Position IS NOT NULL AND Admission2019.Applications.Id in (SELECT ApplicationId From Admission2019.SubjectChoices Where [Order] > 0) ORDER BY UnitName asc, Position asc"
...
for application in applications:
            for user in users:
                if application.get("UserId") == user.get("Id"):
                    cursor.execute(
                        "INSERT INTO PassedApplicants([Id],[Position],[ApplicationId],[Roll],[UnitName],[TotalMarks],[PhoneNumber],[Name],[Father],[Photo],[QuotaName], [AllottedDepartmentId], [AllottedDepartmentOrder], [IsAdmissionConfirmed], [IsAutoMigrationOff], [IsAdmissionCancelled]) values (?,?, ?, ?,?, ?, ?,?, ?, ?,?, ?,?,?,?,?)",
