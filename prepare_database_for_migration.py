def prepare_database(connection_string, unit):
    ...
    create_departments_table = 'CREATE TABLE Departments' + unit + '([Id] [int] NOT NULL,[Code] [nvarchar](200) NOT NULL,[DepartmentName] [nvarchar](200) NOT NULL,[UnitName] [nvarchar](10) NULL,[TotalSeats] [int], [AllottedSeats] [int], [SeatStatus] [bit], [UpdatedDate] [nvarchar](450) NULL)'
    ...
    for application in applications:
        for user in users:
            if application.get("UserId") == user.get("Id"):
                cursor.execute(
                    "INSERT INTO PassedApplicants" + unit + "([Id],[Position],[ApplicationId],[Roll],[UnitName],[TotalMarks],[PhoneNumber],[Name],[Father],[Photo],[QuotaName], [AllottedDepartmentId], [AllottedDepartmentOrder], [IsAdmissionConfirmed], [IsAutoMigrationOff], [IsAdmissionCancelled]) values (?,?, ?, ?,?, ?, ?,?, ?, ?,?, ?,?,?,?,?)",
    ...
    for subject in subjects:
        cursor.execute(
            "INSERT INTO Departments" + unit + " ([Id],[Code],[DepartmentName],[UnitName],[TotalSeats],[AllottedSeats],[SeatStatus]) values (?,?,?,?,?,?,?)",
